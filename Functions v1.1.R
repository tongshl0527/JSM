...
event_sampling <- function(input_data, event =0, testmethod ="chitest") {
  ...
  for (radius in radii) {
      ...
      if (is.null(expVal)==TRUE) {
        ...
        sampled_df <- sampling_of_bestComb (input_data, bestComb_all_intervals, event, radius)
        
        if (empty(sampled_df)== TRUE) {
          comb_df <- comb_df 
        } else {
          sampled_df$radius <- radius
...
sample_index_all_radii <- function(input_sampled_dataset) {
...
get_best_radius_new <- function(input_data, #preprocessed dataset
  ...
  radii_1 <- get_radiusRange(input_data,1) 
    ...
    sampled_0 <- event_sampling(input_data, 0, testmethod)
    stat0_list <- sample_index_all_radii(sampled_0)
    
    sampled_1 <- event_sampling(input_data, 1, testmethod)
    stat1_list <- sample_index_all_radii(sampled_1)
    
    stat0_radii <- sort(unique(sampled_0$radius))
    stat1_radii <- sort(unique(sampled_1$radius))
